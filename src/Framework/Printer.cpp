...
namespace Printer {
	...
	void Empty(std::ostringstream &stream) { stream.str(std::string()); }
	...
	std::string Hex(uintptr_t decimal, EWidthTypes width) { return Hex(decimal, static_cast<uint32_t>(width)); }

	std::string Hex(void *pointer) { return Hex(reinterpret_cast<uintptr_t>(pointer), sizeof(uintptr_t)); }
	...
	std::string Decimal(uintptr_t hex, EWidthTypes width) { return Decimal(hex, static_cast<uint32_t>(width)); }
	...
	void Header(std::ostringstream &stream, const std::string &fileName, const std::string &fileExtension, bool bPragmaPush) {
		...
		stream << "/*\n";
		stream << "#############################################################################################\n";
		stream << std::format("# {} SDK ({}) {}\n", GConfig::GetGameNameLong(), GConfig::GetGameNameShort(), GConfig::GetGameVersion());
		...
		if (bPragmaPush) {
			stream << "\n#ifdef _MSC_VER\n";
			stream << std::format("#pragma pack(push, {})\n", Hex(GConfig::GetFinalAlignment(), 1));
			stream << "#endif\n";
		}
	}
