...
namespace std
{
template <> struct hash<UnrealObject>
{
	size_t operator()(const UnrealObject& unrealObj) const { return hash<string>()(unrealObj.Hash()); }
};

template <> struct hash<UnrealProperty>
{
	size_t operator()(const UnrealProperty& unrealProp) const { return hash<string>()(unrealProp.Hash()); }
};
} // namespace std

// Caches all need objects and strings for sdk generation, caching is only
// needed to be done one time on initialization.
class GCache
{
private:
...
// This is only used for the log file if you have "NO_LOGGING" comment out in
// your config.
class GLogger
{
private:
...
void GetAllObjectFlags(std::ostringstream& stream, uint64_t objectFlags); // Not currently used by anything, but could be
...
{
struct AddPaddingInfo
{
	size_t  accumulatedPropSize = 0;
	int32_t paddingIndex        = 0;
	int32_t prevPropEndOffset   = 0;
};
...
std::filesystem::path CopyFolderWithSuffix(const std::filesystem::path& folder_path, const std::string& suffix = "_w_pch_includes");
} // namespace PostGeneration
