	...
	const std::string FQWord_Struct = R"---(struct FQWord
{
	...
	const std::string UObject_Concept = R"---(template <typename T>
concept UObjectOrDerived = std::is_base_of_v<UObject, T>;
	...
	const std::string UObject_CustomDeclarations =
...
enum EFunctionFlags : uint64_t
{
	...
	FUNC_AllFlags           = 0xFFFFFFFF
	    ...
	    "\tCPF_EditFixedSize =\t\t\t\t\t\t0x0000000000000040, // Indicates that elements of an array can be modified, but its size cannot "
	    "be "
	    "changed.\n"
	    ...
	    "\tCPF_AlwaysInit =\t\t\t\t\t\t0x0000000000100000,\t// Property should never be exported as NoInit(@todo - this doesn\'t need to "
	    "be a "
	    ...
	    "commandlet).\n"
	    "\tCPF_RepRetry =\t\t\t\t\t\t\t0x0000002000000000, // Property replication of this property if it fails to be fully sent (e.g. "
	    "object "
	    ...
	    "\tCPF_CrossLevelActive =\t\t\t\t\t0x0000200000000000, // Property can point across levels, and will be serialized properly, and "
	    "will "
	    ...
	    "\tRF_MisalignedObject =\t\t\t\t\t0x0000000000002000, // Object\'s size no longer matches the size of its C++ class (only used "
	    "during "
	    ...
	    "\tRF_PendingFieldPatches =\t\t\t\t0x0000000000200000, // @script patcher: indicates that this struct will receive additional "
	    "member "
	    ...
	    "\tRF_DisregardForGC =\t\t\t\t\t\t0x0000008000000000,\t// Object is being disregard for GC as its static and itself and all "
	    "references "
	    "are always loaded.\n"
	    ...
	    "\tRF_NeedPostLoadSubobjects =\t\t\t\t0x0000080000000000, // During load, indicates that the object still needs to instance "
	    "subobjects "
	    ...
	    "\tRF_PendingKill =\t\t\t\t\t\t0x2000000000000000, // Objects that are pending destruction (invalid for gameplay but valid "
	    "objects).\n"
	    ...
	    "\tRF_PropagateToSubObjects =\t\t\t\t(RF_Public | RF_ArchetypeObject | RF_Transactional), // Sub-objects will inherit these flags "
	    "from "
	    "their SuperObject.\n"
	    "\n"
	    "\t// custom flags\n"
